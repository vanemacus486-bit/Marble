...
- **源码模式**（默认）—— 基于 CodeMirror 6 的 HTML 编辑器，语法高亮、折叠、搜索，始终可用
- **WYSIWYG 模式**（插件）—— 基于 TipTap（ProseMirror）的富文本编辑器，可在设置中开启
  - 选中文字即可调整字号、颜色、背景色 —— 浮动工具栏即时出现
  - 表格（可调整列宽）、嵌套任务列表、代码块（语法高亮）
  - 文字颜色、字体、高亮、下划线、上下标、对齐
  - 链接、图片、文本样式
- **阅读模式** —— 纯净的 HTML 渲染预览，排版优美
- 右上角图标按钮或 `Ctrl+E` 在可用模式间循环切换
- 多标签页编辑，标签支持拖拽排序
...
### 📤 导出
- **独立 HTML 文件**：右键任意笔记 → "Export as HTML"，生成自包含网页（可直接分享、部署）
- **纯文本**：剥离 HTML 标签
...
- 编辑器：字体、字号、行高、Tab 大小、拼写检查、WYSIWYG 开关、默认编辑模式
...
| **状态管理** | Zustand 5 |
| **编辑器** | CodeMirror 6（源码）+ TipTap 2.11 / ProseMirror（WYSIWYG 插件），30+ 扩展 |
